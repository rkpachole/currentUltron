...
class Index extends Component {
    ...
    calculateShortDays(){
        ...
        console.log("test wll before",attendanceList);
        ...
        console.log("test wll",attendanceList);
    ...
    regularizeAddToScoket() {
        ...
        socket.emit("attendance", { userId: this.state.user.data._id, name: this.state.user.data.first_name });
    }
    componentDidMount() {
        if (this.state.user) {
            const d = new Date();
    ...
    render() {
        const { error, success, loading, CheckRegerror, } = this.props;
        ...
        return (
            <div>
                                    ...
                                    {this.state.filteredKeywords.map((item, index) => (
                                        <tr key={index}>
                                            ...
                                            <td className="">{dateFormat(item.loginDate, "ddd, dd mmm")}
                                                {item.regularizationStatus == "approved" ? <span className="capsule alt_capsule bg-accent-green text-center ml-3 ">REG</span> : ""}</td>
                                            {
                                                ...
                                                item.totalEffectiveTime ?
                                                    ...
                                                    (new Date(item.loginDate).getDay() === 6 || (new Date(item.loginDate).getDay() === 0))
                                                        ?
                                                        <td colSpan="5" className="text-left"> <span className="capsule week_off_bg">Week Off</span></td>
                                                        :
                                                        <td colSpan="5" className="text-left"><span className="capsule lable_weekoff">No Time Entries Logged</span></td>
                                                            ...
                                                            <React.Fragment>
                                                                <div>
                                                                    ...
                                                                    </Button>
                                                                    {
                                                                        ...
                                                                        >
                                                                            <PopoverHeader>
                                                                                {dateFormat(item.loginDate, "ddd, dd mmm  yyyy")}
                                                                            </PopoverHeader>
                ...
                <Modal size="md" isOpen={this.state.regularizeModal} toggle={() => this.closeRegularizeModal()}>
                    ...
                    <ModalHeader className="header-less ml-3" toggle={() => this.closeRegularizeModal()} >
